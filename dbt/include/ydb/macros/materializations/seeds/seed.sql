{% macro ydb__create_csv_table(model, agate_table) %}
  ...
  {%- for col_name in agate_table.column_names -%}
    ...
    {%- set column_definition = adapter.quote_seed_column(column_name, quote_seed_column) ~ ' ' ~ type -%}
    {%- if is_primary_key -%}
      {%- set column_definition = column_definition ~ ' NOT NULL' -%}
    {%- endif -%}
    {%- do column_definitions.append(column_definition) -%}
  {%- endfor -%}
